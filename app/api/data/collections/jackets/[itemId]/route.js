...
export async function GET(req, { params }) {
  ...
  // غير "jackets" باسم الكولكشن الحالي
  const collection = db.collections.jackets;
...
export async function POST(req) {
  ...
  db.collections.jackets.push(itemWithId);
...
export async function PUT(req, { params }) {
  ...
  const index = db.collections.jackets.findIndex(
  ...
  db.collections.jackets[index] = {
    ...db.collections.jackets[index],
    ...updates,
  };

  writeData(db);
  return NextResponse.json(db.collections.jackets[index]);
}
...
export async function DELETE(req, { params }) {
  ...
  const exists = db.collections.jackets.some(
    (el) => String(el.id) === String(itemId)
  );
  if (!exists)
    return NextResponse.json({ error: "Item not found" }, { status: 404 });

  db.collections.jackets = db.collections.jackets.filter(
    (el) => String(el.id) !== String(itemId)
  );
