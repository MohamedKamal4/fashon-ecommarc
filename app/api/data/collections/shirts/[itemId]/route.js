...
export async function GET(req, { params }) {
  ...
  // غير "shirts" باسم الكولكشن الحالي
  const collection = db.collections.shirts;
...
export async function POST(req) {
  ...
  db.collections.shirts.push(itemWithId);
...
export async function PUT(req, { params }) {
  ...
  const index = db.collections.shirts.findIndex(
  ...
  db.collections.shirts[index] = {
    ...db.collections.shirts[index],
    ...updates,
  };

  writeData(db);
  return NextResponse.json(db.collections.shirts[index]);
}
...
export async function DELETE(req, { params }) {
  ...
  const exists = db.collections.shirts.some(
    (el) => String(el.id) === String(itemId)
  );
  if (!exists)
    return NextResponse.json({ error: "Item not found" }, { status: 404 });

  db.collections.shirts = db.collections.shirts.filter(
    (el) => String(el.id) !== String(itemId)
  );
