...
export async function GET(req, { params }) {
  ...
  // غير "hoodies" باسم الكولكشن الحالي
  const collection = db.collections.hoodies;
...
export async function POST(req) {
  ...
  db.collections.hoodies.push(itemWithId);
...
export async function PUT(req, { params }) {
  ...
  const index = db.collections.hoodies.findIndex(
  ...
  db.collections.hoodies[index] = {
    ...db.collections.hoodies[index],
    ...updates,
  };

  writeData(db);
  return NextResponse.json(db.collections.hoodies[index]);
}
...
export async function DELETE(req, { params }) {
  ...
  const exists = db.collections.hoodies.some(
    (el) => String(el.id) === String(itemId)
  );
  if (!exists)
    return NextResponse.json({ error: "Item not found" }, { status: 404 });

  db.collections.hoodies = db.collections.hoodies.filter(
    (el) => String(el.id) !== String(itemId)
  );
